"""
T09_dl_cmp_rect.py — Table 9（tab:dl-cmp-rect）核验
====================================================
对象：R1 矩形域、y=56.1 m 深度线上五种方法的逐频 TL-MAE (dB)，
      6 列：No. / Method / 25Hz / 50Hz / 75Hz / 100Hz，表头带各频源坐标。

这张表**不在 xlsx 里**，源是成图脚本 `advantage_depth_line.py` 从 ep200 npz
的现场提取（组 `comparison_R1_model_advantage`），故 caption 标 last epoch。
核验链与 Table 4/6/7/8 完全不同：
  ...
  H. 图表同源      论文 Fig.12 的 PDF 与 MAE 表出自同一次运行，md5 比对（★）
  I. 加粗正确性    Best in bold 须真的落在每列最小值上
  J. 跨表版式      与 Table 10 列定义/表头一致（并列 minipage，须等宽）
  K. 正文引用      1.515 / y=56.1 / DeepONet exceeds 5 dB
"""
import os
import re
...
SLUG = "T09_dl_cmp_rect"
REC = registry.by_slug(SLUG)
LABEL = REC["label"]
GROUP = "comparison_R1_model_advantage"
SIB = "tab:dl-cmp-wedge"
FIG_LABEL = "fig:dl-cmp-rect"
FIG_FILE = "comparison_R1_model_advantage.pdf"
...
def run():
    ...
    c.source("成图/取数脚本（权威）", DL.AUTH, "组 `comparison_R1_model_advantage`")
    ...
    c.note("脚本用 `ROOT = dirname(__file__)` 定位数据与产物，只有位于 "
           "`D:\\Data` 下才能同时命中 `ROOT/Case15-24` 与 `ROOT/重绘结果`；"
    ...
    for name, got, want in (("插值网格 GRID", m.GRID, 300),
                            ...
                            ("指定深度线 force_y", cfg.get("force_y"), 56.1),
                            ("数据目录 grpdir", cfg["grpdir"], "Case15-24"),
                            ("域类型", cfg["domain"], "Rectangle")):
    ...
    c.note(f"重算落在第 {R['row']} 行，实际深度 y={R['y_line']:.6f} m；"
           f"force_y={cfg['force_y']} 取最近行，caption 写 56.1 m 是其一位小数。")
    c.check(abs(round(R["y_line"], 1) - 56.1) < 1e-9,
            "选中行深度舍入到 1 位 = 56.1 m", f"实际 `{R['y_line']:.6f}`")
    cap = T.caption_of(LABEL) or ""
    c.check("y=56.1" in cap.replace("$", "").replace("\\,", "").replace(" ", ""),
            "caption 深度值与重算一致", "caption 含 `y=56.1\\,m`")
    ...
    c.check(set(printed) == {r[0] for r in ROWS}, "行 No. 覆盖 Case 15-19",
            str(sorted(printed)))
    ...
    # ── H ────────────────────────────────────────────────────────
    c.section("9. 表与图同源（Table 9 ↔ Fig. 12）")
    ...
    c.check("56.1" in figcap, "图注深度与表一致", "图注含 `y=56.1\\,m`")
    ...
    # ── J ────────────────────────────────────────────────────────
    c.section("12. 与 Table 10 的版式一致性")
    ...
    # 四表总宽须相同：Tables 9-12 两两并排，宽度不一致时边缘对不齐。
    # tabular* 的宽度参数就是总宽，四处必须都是 \TABDLW。
    import re as _re
    widths = {}
    for lb in ("tab:dl-cmp-rect", "tab:dl-cmp-wedge",
               "tab:dl-abl-rect", "tab:dl-abl-wedge"):
        e = T.table_env(lb) or ""
        m = _re.search(r"\\begin\{tabular\*\}\{([^}]*)\}", e)
        widths[lb] = m.group(1) if m else None
    same = len(set(widths.values())) == 1 and None not in widths.values()
    c.check(same, "Tables 9-12 的 tabular* 总宽参数一致（四表等宽）",
            " / ".join(f"{k.split(':')[1]}=`{v}`" for k, v in widths.items()))

    # ── K ────────────────────────────────────────────────────────
    c.section("13. 正文引用精确性（4.4 节）")
    prop = [R["er"][f][0] for f in FREQS]
    worst = max(prop)
    c.check(f"{worst:.3f}" == "1.515", "正文『at or below 1.515 dB』= 本文法最大值",
            f"四频 {[f'{v:.3f}' for v in prop]} → 最大 `{worst:.3f}`")
    c.eq("正文 1.515 ← 全精度源", worst, "1.515")
    txt = T.tex_text()
    hits = T.sentences_with(r"y=56\.1", txt)
    c.check(bool(hits), "正文声明的深度线 y=56.1 m 与脚本 force_y 一致",
            f"tex 行 {T.line_of(hits[0][0])}" if hits else "未找到")
    dn = max(R["er"][f][1] for f in FREQS)
    ...
    c.section("14. 本文法逐频占优")
    for f in FREQS:
        col = R["er"][f]
        c.check(col[0] == min(col), f"{f}Hz Proposed 为最小",
