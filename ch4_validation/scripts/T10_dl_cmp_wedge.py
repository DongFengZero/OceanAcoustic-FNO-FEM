"""
T10_dl_cmp_wedge.py — Table 9（tab:dl-cmp-wedge）核验
====================================================
对象：W1 楔形域、y=30.4 m 深度线上五种方法的逐频 TL-MAE (dB)，
      6 列：No. / Method / 25Hz / 50Hz / 75Hz / 100Hz，表头带各频源坐标。

这张表**不在 xlsx 里**，源是成图脚本 `advantage_depth_line.py` 从 ep200 npz
的现场提取（组 `comparison_W1_model_advantage`），故 caption 标 last epoch。
核验链与 Table 4/6/7/8 完全不同：
  ...
  K. 正文引用      0.666 / y=30.4 / DeepONet exceeds 5 dB
"""
import os
import re
...
SLUG = "T10_dl_cmp_wedge"
REC = registry.by_slug(SLUG)
LABEL = REC["label"]
GROUP = "comparison_W1_model_advantage"
SIB = "tab:dl-cmp-wedge"
FIG_LABEL = "fig:dl-cmp-wedge"
FIG_FILE = "comparison_W1_model_advantage.pdf"

# tex 行序 → (No., Method 印刷名, 脚本内方法标签)
ROWS = [
    (20, "Proposed", "Proposed (Ours)"),
    (21, "DeepONet", "DeepONet"),
    (22, "FNO", "FNO"),
    (23, "KNO", "KNO"),
    (24, "CNO", "CNO"),
]
...
def run():
    ...
    c.source("成图/取数脚本（权威）", DL.AUTH, "组 `comparison_W1_model_advantage`")
    ...
    for name, got, want in (("插值网格 GRID", m.GRID, 300),
                            ...
                            ("指定深度线 force_y", cfg.get("force_y"), 30.4),
    ...
    c.note(f"重算落在第 {R['row']} 行，实际深度 y={R['y_line']:.6f} m；"
           f"force_y={cfg['force_y']} 取最近行，caption 写 30.4 m 是其一位小数。")
    c.check(abs(round(R["y_line"], 1) - 30.4) < 1e-9,
            "选中行深度舍入到 1 位 = 30.4 m", f"实际 `{R['y_line']:.6f}`")
    cap = T.caption_of(LABEL) or ""
    c.check("y=30.4" in cap.replace("$", "").replace("\\,", "").replace(" ", ""),
            "caption 深度值与重算一致", "caption 含 `y=30.4\\,m`")
    ...
    c.check(set(printed) == {r[0] for r in ROWS}, "行 No. 覆盖 Case 20-19",
            str(sorted(printed)))
    ...
    c.check("30.4" in figcap, "图注深度与表一致", "图注含 `y=30.4\\,m`")
    ...
    worst = max(prop)
    c.check(f"{worst:.3f}" == "0.666", "正文『at or below 0.666 dB』= 本文法最大值",
            f"四频 {[f'{v:.3f}' for v in prop]} → 最大 `{worst:.3f}`")
    c.eq("正文 0.666 ← 全精度源", worst, "0.666")
    txt = T.tex_text()
    hits = T.sentences_with(r"y=56\.1", txt)
    c.check(bool(hits), "正文声明的深度线 y=30.4 m 与脚本 force_y 一致",
