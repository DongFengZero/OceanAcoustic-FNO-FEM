"""
T11_dl_abl_rect.py — Table 11（tab:dl-abl-rect）核验
====================================================
对象：R1 矩形域、y=71.9 m 深度线上四种消融变体的逐频 TL-MAE (dB)，
      6 列：No. / Method / 25Hz / 50Hz / 75Hz / 100Hz，表头带各频源坐标。

这张表**不在 xlsx 里**，源是成图脚本 `advantage_depth_line.py` 从 ep200 npz
的现场提取（组 `ablation_R1_module_advantage`），故 caption 标 last epoch。
核验链与 Table 4/6/7/8 完全不同：
  ...
  E. 补 0 判别     json 只存 round(er,3)，`0.540` 真伪必须靠全精度裁定（★）
  F. 印刷值        16 格逐字符比对
  G. 表头源坐标    4 个 (x,y) 与所选样本的 source_pos 一致（★）
  H. 图表同源      论文 Fig.12 的 PDF 与 MAE 表出自同一次运行，md5 比对（★）
  I. 加粗正确性    Best in bold 须真的落在每列最小值上——本表 25 Hz 的最小值
                   落在 w/o prior-sup.（0.540）而非 Full，加粗必须跟着数走
  J. 跨表版式      与 Table 12 列定义/表头一致（并列 minipage，须等宽）
...
SLUG = "T11_dl_abl_rect"
REC = registry.by_slug(SLUG)
LABEL = REC["label"]
GROUP = "ablation_R1_module_advantage"
SIB = "tab:dl-abl-wedge"
FIG_LABEL = "fig:dl-abl-rect"
FIG_FILE = "ablation_R1_module_advantage.pdf"
...
def run():
    ...
    c.source("成图/取数脚本（权威）", DL.AUTH, "组 `ablation_R1_module_advantage`")
    ...
    for name, got, want in (("插值网格 GRID", m.GRID, 300),
                            ...
                            ("指定深度线 force_y", cfg.get("force_y"), 71.9),
    ...
    c.note(f"重算落在第 {R['row']} 行，实际深度 y={R['y_line']:.6f} m；"
           f"force_y={cfg['force_y']} 取最近行，caption 写 71.9 m 是其一位小数。")
    c.check(abs(round(R["y_line"], 1) - 71.9) < 1e-9,
            "选中行深度舍入到 1 位 = 71.9 m", f"实际 `{R['y_line']:.6f}`")
    cap = T.caption_of(LABEL) or ""
    c.check("y=71.9" in cap.replace("$", "").replace("\\,", "").replace(" ", ""),
            "caption 深度值与重算一致", "caption 含 `y=71.9\\,m`")
    ...
    c.note("判定用全精度值，不用 json —— json 已是 round(...,3)，"
           "拿它比对等于自证，无法识别补 0（如 w/o prior-sup.@25Hz 印 `0.540`，"
           "全精度 0.540375313 才是真值来源）。")
    ...
    c.check(set(printed) == {r[0] for r in ROWS}, "行 No. 覆盖 Case 25-28",
            str(sorted(printed)))
    ...
    # ── H ────────────────────────────────────────────────────────
    c.section("9. 表与图同源（Table 11 ↔ Fig. 12）")
    ...
    c.check("71.9" in figcap, "图注深度与表一致", "图注含 `y=71.9\\,m`")
    ...
    # ── J ────────────────────────────────────────────────────────
    c.section("12. 与 Table 12 的版式一致性")
    sib = T.table_env(SIB) or ""
    a, b = T.tabular_preamble(env), T.tabular_preamble(sib)
    c.check(a is not None and a == b, "列定义与 Table 12 相同",
            f"Table 11 `{a}` / Table 12 `{b}`")
    # 这一族改用 tabular* 固定总宽（四表等宽），故前导是 \extracolsep{\fill}
    # 而非 @{}；断言列类型序列本身，不锁整串字面。
    c.check(a is not None and a.endswith("QA EEEE@{}"),
            "列类型序列为消融深度线族专用 `QA EEEE`", f"`{a}`")
    ...
    hits = T.sentences_with(r"y=71\.9", txt)
    c.check(bool(hits), "深度线深度 y=71.9 m 在 caption 中声明且与脚本 force_y 一致",
            f"tex 行 {T.line_of(hits[0][0])}" if hits else "未找到")

    # ── L ────────────────────────────────────────────────────────
    c.section("14. 消融结论方向性（去掉模块应变差）")
    c.note("消融表不要求 Full 逐频最优：25 Hz 最小值落在 w/o prior-sup.（0.540），"
           "这是真实结果，加粗已按列最小值标注。此处只断言物理先验是主导项。")
    for f in FREQS:
        col = R["er"][f]
    ...
    c.check(wins == 3, "Full 在 4 个频率中占优 3 个（25 Hz 除外）",
            f"占优频率数 {wins}")
