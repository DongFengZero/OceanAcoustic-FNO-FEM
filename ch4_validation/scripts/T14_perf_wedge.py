"""
T14_perf_wedge.py — Table 13（tab:perf-wedge）核验
====================================================
...
PROSE = [
    ("Case 20 Proposed 频均 Sol", "2.121", (20, "Overall", "sol")),
    ("Case 20 Proposed 频均 TL", "0.899", (20, "Overall", "tl")),
    ("Case 22 FNO 频均 Sol", "3.179", (22, "Overall", "sol")),
    ("Case 22 FNO 频均 TL", "1.090", (22, "Overall", "tl")),
    ("Case 20 @100Hz TL", "1.265", (20, 100, "tl")),
    ("Case 21 DeepONet @100Hz TL", "5.512", (21, 100, "tl")),
]
...
def run():
    ...
    c.check(set(printed) == set(CASES), "tex 行 No. 覆盖 20-24",
            str(sorted(printed)))
    ...
    for no in CASES:
        c.check(printed[no][1].strip() == CASES[no][0], f"Case {no} Method 名",
    ...
    c.check(not bad, "全部 50 个数值单元格均为 3 位小数",
            "全部合规" if not bad else "；".join(bad))

    # ── G ────────────────────────────────────────────────────────
    c.section("8. 正文引用精确性（4.4 节）")
