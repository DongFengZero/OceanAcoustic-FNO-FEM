"""
T19_gen_overall.py — Table 19（tab:gen-overall）核验
====================================================
...
# 正文直接引用（4.7 节泛化段落）
PROSE = [
    ("R9 频均 TL", "3.642", (39, "Overall", "tl")),
    ("R10 频均 TL", "2.966", (40, "Overall", "tl")),
    ("W9 频均 TL", "4.347", (41, "Overall", "tl")),
    ("W10 频均 TL", "4.437", (42, "Overall", "tl")),
]
...
def run():
    ...
    c.check(set(printed) == set(CASES), "tex 行 No. 覆盖 39-42",
            str(sorted(printed)))
    ...
    c.note("列序：No., Data, Extrap.region, 25Hz(Sol,TL), 50Hz, 75Hz, 100Hz, Avg.(Sol,TL)。"
           "Avg. 对应 xlsx/日志的 Overall 组。"
           "日志渠道的一致性已在第 4 节双渠道验证中确认，此处仅比对 xlsx。")
    order = [(25, 0), (50, 1), (75, 2), (100, 3), ("Overall", 4)]
    for no in CASES:
        c.check(printed[no][1].strip() == CASES[no][0], f"Case {no} Data 名",
                f"tex `{printed[no][1].strip()}`")
        for g, blk in order:
            for k, q in enumerate(("sol", "tl")):
                cell = printed[no][3 + blk * 2 + k]  # 数值从第 4 列开始（索引 3）
    ...
    bad = []
    for no in CASES:
        for j in range(3, 13):  # 数值列：索引 3–12（第 4–13 列）
            v = printed[no][j]
